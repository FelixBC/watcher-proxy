...
echo [2/7] Installing to run at Windows logon...
...
schtasks /create /tn "Watcher Proxy Loop" /tr "wscript \"%BRAIN_DIR%\RunWatchdogLoopHidden.vbs\"" /sc onlogon /delay 0000:15 /f >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    echo        [OK] If proxy dies: normal internet within 5 sec, proxy auto-restarts ^(or run WatcherBrain\RestoreInternetNow.bat^)
) else (
    echo        [WARNING] Watcher Proxy Loop task may not have been created
)
...
echo [3/7] Adding antivirus exclusion ^(so antivirus doesn't remove the proxy^)...
...
echo [4/7] Scheduling print spool cleanup ^(at logon, once per day^)...
...
echo [5/7] Starting proxy server...
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo        Try running WatcherBrain\StartWatcher.vbs manually
)
timeout /t 1 /nobreak >nul

REM Step 6: Configure Windows proxy (manual 127.0.0.1:8080, auto-detect OFF)
echo [6/7] Configuring proxy settings...
